...
export default class DaysGridView extends Component {
  ...
  render() {
    ...
    const renderedDaysGrid = daysGrid.map((weekRow, i) => {
      let rowBlank = false;
      let datm = weekRow.map((day) => {
        rowBlank = rowBlank || day?.day ? true : false;
        return day.component;
      });

      if (i == 0 && rowBlank == false) return null;
    ...
    return (
      <View style={styles.daysWrapper}>
        <View style={styles.monthYearHeaderWrapper}>
          ...
          <Text styles={styles.yearHeaderMainText}> {this.props.year}</Text>
        </View>
        {renderedDaysGrid}
      </View>
    );
